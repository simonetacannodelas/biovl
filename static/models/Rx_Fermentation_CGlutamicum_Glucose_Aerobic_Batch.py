...
class CGlutamicum_aerobic:
    ...
    def rxn(self, C,t , u, fc):
        ...
        r=np.zeros((5,1))
        if C[1] <= self.Pcritical:
            r[0,0]=((self.mu_max*C[2])/(C[2]+self.Kg))*((1+(C[1]/self.Pcritical))**self.nu)*C[0]
        elif C[1] > self.Pcritical:
            r[0,0] = - self.kd*C[0]

        r[1,0]=self.Ypx*r[0,0]
        r[2,0]=-(1/self.Yxs)*r[0,0]-(1/self.Yps)*r[1,0]
    ...
    def create_plot(self, t, C):
        ...
        P = C[:, 1]
        G = C[:, 2]
        df = pd.DataFrame({'t': t, 'G': G, 'B': B, 'P': P})
        fig = go.Figure()
        fig.add_trace(go.Scatter(x=df['t'], y=df['G'], name='Glucose'))
        fig.add_trace(go.Scatter(x=df['t'], y=df['B'], name='Biomass'))
        fig.add_trace(go.Scatter(x=df['t'], y=df['P'], name='L-Glutamic'))
        fig.update_layout(title=('Simulation of the model for Corynebacterium glutamicum'),
                          xaxis_title='time (h)',
...
#
#         l1, = ax.plot(time, data[:len(time),0], color = 'g',label = 'Corynebacterium glutamicum')
