...
class MSucciniciproducens_anae:
    def __init__(self, Control = False):
        ...
        self.u_max = 150
        self.u_min = 0
    def Compounds(self, C, t):
        dictCompounds={'Manheimia succiniciproducens': C[:,0],
                       'Succinic acid':C[:,1],
                       'Acetic acid':C[:,2],
                       'Formic acid':C[:,3],
                       'Lactic acid':C[:,4],
                       'Glucose':C[:,5]}
        return(dictCompounds)
    ...
    def create_plot(self, t, C):
        ...
        df = pd.DataFrame({'t': t, 'G': G, 'B': B, 'SA': SA, 'AA':AA, 'FA':FA, 'LA':LA})
        fig = go.Figure()
        fig.add_trace(go.Scatter(x=df['t'], y=df['G'], name='Glucose'))
        fig.add_trace(go.Scatter(x=df['t'], y=df['B'], name='Biomass'))
        fig.add_trace(go.Scatter(x=df['t'], y=df['SA'], name='Succinic Acid'))
        fig.add_trace(go.Scatter(x=df['t'], y=df['AA'], name='Acetic acid'))
        fig.add_trace(go.Scatter(x=df['t'], y=df['FA'], name='Fumeric acid'))
        fig.add_trace(go.Scatter(x=df['t'], y=df['LA'], name='Lactic acid'))
        fig.update_layout(title=('Simulation of the model for a succinic acid bacteria'),
                          xaxis_title='time (h)',
...
#        
#        l1, = ax.plot(time, data[:len(time),0], color = 'g',label = dictCompound.keys()[0])
...
#plt.plot(f.solve()[0],f.solve()[1][:,5])
##plt.plot(f.solve()[0], C[5])
#plt.show()
